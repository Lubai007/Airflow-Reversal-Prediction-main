...
class NodeDataLoader(DataLoader):
    ...
    def _combine_node_connection(self, node_matries:list | tuple):
        ...
        for i in node_matries:
            matrix_size += i.size(0)-1 # Calculates and sets the end index (end).
        combined_matrix = torch.zeros(matrix_size+1, matrix_size+1).to(self.device) # Create a zero matrix of size (matrix_size+1) x (matrix_size+1) and move it to the same device as self.device.

        # Iterate through each node connection matrix, copying it (excluding the last row and column) into the combined matrix
        start = 0
        for node_matrix in node_matries:
            end = start + node_matrix.size(0)-1
            combined_matrix[start:end, start:end] += node_matrix[:-1,:-1]
            start = end
        outdoor_connection = torch.cat([matrix[:-1,-1] for matrix in node_matries]) # Extract the last column (excluding the last element) from each node connection matrix and concatenate them into a new vector.
        outdoor_connection = torch.cat([outdoor_connection, torch.ones(1).to(self.device)]) # Append a 1 to the end of outdoor_connection (representing the connection to the outdoor environment for all nodes).
